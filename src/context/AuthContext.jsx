            ...
            if (currentUser) {
                ...
                const roleUnsubscribe = onSnapshot(userDocRef, (docSnapshot) => {
                    if (docSnapshot.exists()) {
                        ...
                        // User document does not exist yet (first login or not created)
                        // Check for hardcoded admin emails
                        const adminEmails = ['siienergia.pdm@gmail.com', 'servidorsiiewcim@gmail.com', 'ludu8d@gmail.com'];
                        if (adminEmails.includes(currentUser.email)) {
                            console.warn(`User document not found for: ${currentUser.email}, defaulting to 'admin' (Hardcoded)`);
                            setUserRole('admin');
                        } else {
                            console.warn(`User document not found for: ${currentUser.email}, defaulting to 'tecnico'`);
                            setUserRole('tecnico');
                        }
                    }
                }, (err) => {
